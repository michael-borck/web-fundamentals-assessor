...
def run_script(command_parts, tool_name, student_main_output_dir, log_file_path):
    ...
    if command_parts[0].lower() != 'python' and not command_parts[0].lower().endswith('python.exe'):
        if os.path.isfile(os.path.join(SCRIPTS_DIR, command_parts[0])): # If script itself is first
             command_parts.insert(0, 'python') # Prepend python
        # Add more checks if necessary for other ways scripts might be called
    ...
    with open(log_file_path, 'a', encoding='utf-8') as log_f:
        ...
        try:
            ...
            error_msg = f"Error: The script for {tool_name} (e.g., '{command_parts[1]}') or the Python interpreter was not found. Ensure SCRIPTS_DIR is correct and scripts are present, and Python is in PATH."
            print(error_msg)
...
def prepare_ai_conversations(manual_conversations_folder_path, chat_scrape_file_path, target_input_dir, log_file_path, headless_scraping=True):
    ...
    if manual_conversations_folder_path and os.path.isdir(manual_conversations_folder_path):
        ...
        for item in os.listdir(manual_conversations_folder_path):
            ...
            if os.path.isfile(source_item_path):
                try:
                    ...
                    print(f"Copied {item} to {target_input_dir}")
                except Exception as e:
        ...
        print("No manual conversations folder provided or it's not a valid directory.")
    ...
    if chat_scrape_file_path and os.path.exists(chat_scrape_file_path):
        ...
        cmd = ['python', batch_scrape_script_path,
        ...
        if run_script(cmd, "batch_scrape_conversations.py", os.path.dirname(target_input_dir), log_file_path):
            ...
            if os.path.exists(failed_log):
                with open(failed_log, 'r', encoding='utf-8') as f_err:
                    failed_urls = f_err.read()
                if failed_urls.strip():
...
def clone_git_repository(repo_url, clone_target_dir, log_file_path):
    ...
    if os.path.exists(clone_target_dir) and os.listdir(clone_target_dir):
        print(f"Git repository folder {clone_target_dir} already exists and is not empty. Assuming cloned. Skipping clone.")
        return True
    elif os.path.exists(clone_target_dir) and not os.listdir(clone_target_dir):
...
def aggregate_final_report(student_id, student_main_output_dir, dirs_config, final_report_path, rubric_file_path):
    ...
    for section_name, (tool_script, report_pattern) in REPORT_MAPPING.items():
        ...
        if tool_script == "conversation_analyser.py":
            ...
            # Fallback or define more specific keys if needed
            tool_output_dir_key = tool_script.replace('_analyser.py', '_reports').replace('_analyzer.py', '_reports').replace('_checker.py', '_reports')

        tool_report_dir_path = dirs_config.get(tool_output_dir_key)

        if not tool_report_dir_path:
            print(f"Warning: Output directory key '{tool_output_dir_key}' not found in dirs_config for tool '{tool_script}'. Skipping report for '{section_name}'.")
            continue
        
        report_files_found = glob.glob(os.path.join(tool_report_dir_path, report_pattern))
    ...
    with open(final_report_path, 'w', encoding='utf-8') as f_final:
        ...
        f_final.write("\n## Aggregated Assessment Results by Rubric Category\n\n")
        ...
        for rubric_cat in RUBRIC_SECTIONS_ORDER:
            ...
            if not found_content_for_category:
                # Add placeholder messages for sections not covered by automated reports
                if rubric_cat == "Final Product & Technical Functionality":
                     f_final.write("*(Partially covered by Performance Analysis. Functionality of embedded projects typically requires manual review.)*\n\n")
                elif rubric_cat == "Design & Responsiveness": # Check specific sub-items
                     f_final.write("*(Responsive Design and Flexbox/Grid usage are covered by `responsive_analyser.py`. Cohesive color scheme and typography typically require manual review.)*\n\n")
                elif rubric_cat == "Development Process":
                     f_final.write("*(Code Organisation/Documentation and Validation are covered by `code_quality_analyser.py` and `validate_web.py`. Problem-solving approaches may require insights from Git history, AI conversations, and manual review.)*\n\n")
                else:
                    f_final.write("*(No automated reports directly mapped to this primary rubric section, or relevant reports were not found. Manual review may be required, or check individual tool outputs.)*\n\n")
            f_final.write("---\n\n")
...
def main():
    ...
    parser.add_argument("--website_folder", required=True, help="Path to the student's unzipped website project folder.")
    ...
    parser.add_argument("--chrome_path", help="Optional: Path to Chrome/Chromium executable for Lighthouse/Selenium.")
    ...
    if not clone_git_repository(args.git_repo_url, git_repo_local_path, log_file_path):
        print(f"CRITICAL: Failed to clone Git repository. Git-dependent analyses will be affected.")
    ...
    if args.chrome_path: # Assuming screenshot.py uses selenium which might pick up chrome from PATH or use its own logic
        pass # ChromeDriver should be in PATH or handled by selenium if chrome_path is not a direct arg to screenshot.py
    ...
    dep_cmd = ['python', os.path.join(SCRIPTS_DIR, 'deployment_analyser.py'),
               args.git_repo_url, args.netlify_url,
    ...
    if os.path.exists(git_repo_local_path) and os.listdir(git_repo_local_path): # Check if clone was successful
        git_cmd = ['python', os.path.join(SCRIPTS_DIR, 'git_analyser.py'),
                   os.path.abspath(git_repo_local_path),
    ...
    try:
        if os.path.exists(git_repo_local_path):
            ...
            shutil.rmtree(git_repo_local_path)
