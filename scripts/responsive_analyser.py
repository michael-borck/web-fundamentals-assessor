...
class ResponsiveDesignAnalyzer:
    ...
    def generate_report(self, results, output_file):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"### Effective Use of Flexbox/Grid (4%)\n\n")
            f.write(f"**Performance Level:** {flexbox_grid_level}\n\n")
            f.write(f"**Score:** {flexbox_grid_score:.2f}/4 points ({flexbox_percentage:.1f}%)\n\n")
            
            f.write("| Performance Level | Description | Points |\n")
            f.write("|-------------------|-------------|--------|\n")
            f.write("| Distinction (75-100%) | Advanced implementation of Flexbox/Grid with optimal use of space and sophisticated layouts | 3-4 |\n")
            f.write("| Credit (65-74%) | Competent use of Flexbox/Grid with well-structured layouts | 2.6-2.99 |\n")
            f.write("| Pass (50-64%) | Basic implementation of Flexbox/Grid with functional layouts | 2-2.59 |\n")
            f.write("| Fail (0-49%) | Inadequate implementation of Flexbox/Grid; layout issues evident | 0-1.99 |\n\n")
    ...
    def find_all_css_files(self, folder_path):
        ...
        Returns:
            Dictionary mapping HTML files to their linked CSS files
        """
        ...
        for html_file in html_files:
            ...
            if not linked_css:
                # If no linked CSS found, use all CSS files (fallback)
                html_to_css[html_file] = all_css_files
            else:
                html_to_css[html_file] = linked_css
    ...
    def find_screenshot_pairs(self, screenshot_dir):
        ...
        # Extract base names (without the .desktop.png or .mobile.png suffix)
        desktop_bases = {os.path.splitext(os.path.splitext(path)[0])[0]: path for path in desktop_screenshots}
        mobile_bases = {os.path.splitext(os.path.splitext(path)[0])[0]: path for path in mobile_screenshots}
